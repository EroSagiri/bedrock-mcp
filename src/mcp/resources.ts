...
export function registerMineralResources(ctx: McpRegistrationContext): void {
  registerResourceCompat(
    ...
    new ResourceTemplate("mineral://doc/{key}", {
      list: async () => ({
        resources: (await listTextObjects(ctx.env.MINERAL)).map(object => ({
      ...
      complete: {
        key: value => completeKeys(ctx.env.MINERAL, value),
      },
    }),
    { description: "读取 vault 中的一篇文本笔记。", mimeType: "text/markdown" },
    async (uri, { key }) => {
      const decodedKey = decodeVar(key);
      const obj = await ctx.env.MINERAL.get(decodedKey);
  ...
  registerResourceCompat(
    ...
    async uri => {
      ...
      let cursor: string | undefined;
      do {
        const result = await ctx.env.MINERAL.list({ cursor, limit: 1000 });
  ...
  registerResourceCompat(
    ...
    async uri => jsonResource(
      uri,
      (await listTextObjects(ctx.env.MINERAL))
  ...
  registerResourceCompat(
    ...
    async uri => jsonResource(
      uri,
      (await collectFolderPrefixes(ctx.env.MINERAL)).map(item => ({
        folder: item.folder,
        count: item.count,
  ...
  registerResourceCompat(
    ...
    new ResourceTemplate("mineral://folder/{path}", {
      list: async () => ({
        resources: (await collectFolderPrefixes(ctx.env.MINERAL)).map(item => ({
      ...
      complete: {
        path: value => completeFolders(ctx.env.MINERAL, value),
      },
    }),
    { description: "按文件夹前缀浏览笔记。", mimeType: "application/json" },
    async (uri, { path }) => {
      const prefix = decodeVar(path).replace(/^\/+/, "").replace(/\/?$/, "/");
      const objects = await listTextObjects(ctx.env.MINERAL, prefix);
      return jsonResource(uri, {
        path: prefix,
  ...
  registerResourceCompat(
    ...
    async uri => jsonResource(uri, await buildGraph(ctx.env.MINERAL, { includeDangling: true, limit: 1000 }))
  );
}
