...
// 简易 YAML frontmatter 提取（不做完整 YAML 解析，只切出原文 + 抓常用字段）
function parseFrontmatter(text: string): { frontmatter: Record<string, string> | null; body: string } {
  const m = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/.exec(text);
...
function snippet(text: string, query: string, ctx = 60): string {
...
function snippetAt(text: string, index: number, length: number, ctx = 60): string {
  const start = Math.max(0, index - ctx);
  const end = Math.min(text.length, index + length + ctx);
  return (start > 0 ? "…" : "") + text.slice(start, end).replace(/\s+/g, " ") + (end < text.length ? "…" : "");
}

type Matcher = { match(s: string): { index: number; length: number } | null };

function buildMatcher(query: string, regex: boolean, caseSensitive: boolean): Matcher | { error: string } {
  if (regex) {
    try {
...
const TEXT_EXTS = [".md", ".txt", ".markdown", ".mdx"];
...
/** 把 R2 key 展开成所有可能被 wikilink 引用的形式 */
function backlinkTargets(key: string): Set<string> {
  const noExt = key.replace(/\.(md|markdown|mdx|txt)$/i, "");
  const basename = noExt.split("/").pop() ?? noExt;
  return new Set([noExt, basename]);
}

/** 扫描整个 bucket 里的文本文件，对每篇调用 fn；fn 返回 null 表示不命中 */
async function scanTextFiles<T>(
  bucket: R2Bucket,
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ prefix, cursor, limit }) => {
        const r = await this.env.BEDROCK.list({
          prefix,
          cursor,
          limit: limit ?? 100,
    ...
    this.server.tool(
      ...
      async () => {
        ...
        let cursor: string | undefined;
        do {
          const r = await this.env.BEDROCK.list({ cursor, limit: 1000 });
    ...
    this.server.tool(
      ...
      async ({ key, raw }) => {
        const obj = await this.env.BEDROCK.get(key);
    ...
    this.server.tool(
      ...
      async ({ query, regex, caseSensitive, searchIn, prefix, limit, contextChars }) => {
        ...
        let cursor: string | undefined;
        scan: do {
          const r = await this.env.BEDROCK.list({ prefix, cursor, limit: 1000 });
          ...
          for (let i = 0; i < candidates.length; i += batchSize) {
            ...
            const results = await Promise.all(batch.map(async o => {
              ...
              if (needsContent && isTextFile(o.key)) {
                const obj = await this.env.BEDROCK.get(o.key);
                if (obj) {
                  const text = await obj.text();
                  ...
                  if (fields.has("content")) {
                    ...
                    if (m) matches.push({
                      in: "content",
                      snippet: snippetAt(text, m.index, m.length, ctx),
                    });
                  }
                  if (fields.has("tags")) {
    ...
    this.server.tool(
      ...
      async ({ date, folder }) => {
        ...
        for (const k of candidates) {
          const obj = await this.env.BEDROCK.get(k);
          if (obj) {
            return ok(JSON.stringify({
              key: k,
              modified: obj.uploaded.toISOString(),
        ...
        const r = await this.env.BEDROCK.list({ prefix: `${dir}/`, limit: 1000 });
    ...
    this.server.tool(
      ...
      async ({ key, content, contentType }) => {
        ...
        const ct = contentType ?? (key.toLowerCase().endsWith(".md") ? "text/markdown; charset=utf-8" : "text/plain; charset=utf-8");
        const existed = await this.env.BEDROCK.head(key);
        await this.env.BEDROCK.put(key, content, {
    ...
    this.server.tool(
      ...
      async ({ key }) => {
        ...
        await this.env.BEDROCK.delete(key);
    ...
    this.server.tool(
      ...
      async ({ keys }) => {
        await this.env.BEDROCK.delete(keys);
    ...
    this.server.tool(
      ...
      async ({ key, content, contentType }) => {
        ...
        const ct = contentType ?? (key.toLowerCase().endsWith(".md") ? "text/markdown; charset=utf-8" : "text/plain; charset=utf-8");
        await this.env.BEDROCK.put(key, content, { httpMetadata: { contentType: ct } });
    ...
    this.server.tool(
      ...
      async ({ key, content, separator, createIfMissing }) => {
        ...
        const ct = obj?.httpMetadata?.contentType
          ?? (key.toLowerCase().endsWith(".md") ? "text/markdown; charset=utf-8" : "text/plain; charset=utf-8");
        await this.env.BEDROCK.put(key, next, { httpMetadata: { contentType: ct } });
        return ok(JSON.stringify({ ok: true, key, action, size: new TextEncoder().encode(next).length }, null, 2));
      }
    );
    ...
    this.server.tool(
      ...
      async ({ key, base64, contentType }) => {
        ...
        await this.env.BEDROCK.put(key, bytes, { httpMetadata: { contentType: ct } });
        return ok(JSON.stringify({
          ok: true,
          key,
    ...
    this.server.tool(
      ...
      async ({ key }) => {
        const obj = await this.env.BEDROCK.head(key);
        if (!obj) return err(`Not found: ${key}`);
        return ok(JSON.stringify({
          key,
          publicPath: `/static/${key}`,
    ...
    this.server.tool(
      ...
      async ({ path }) => {
        ...
        const existed = await this.env.BEDROCK.head(placeholder);
        if (existed) return ok(JSON.stringify({ ok: true, folder, action: "exists" }, null, 2));
        await this.env.BEDROCK.put(placeholder, "", {
    ...
    this.server.tool(
      ...
      async ({ from, to, overwrite }) => {
        ...
        const src = await this.env.BEDROCK.get(from);
        if (!src) return err(`源文件不存在：${from}`);
        if (!overwrite) {
          const dst = await this.env.BEDROCK.head(to);
          if (dst) return err(`目标已存在，传 overwrite: true 强制覆盖：${to}`);
        }
        await this.env.BEDROCK.put(to, src.body, {
          httpMetadata: src.httpMetadata,
          customMetadata: src.customMetadata,
        });
        await this.env.BEDROCK.delete(from);
    ...
    this.server.tool(
      ...
      async ({ key, limit }) => {
        ...
        const matches = await scanTextFiles(this.env.BEDROCK, undefined, (k, text, o) => {
      ...
      async ({ key }) => {
        ...
        const checks = await Promise.all(links.map(async l => {
          ...
          for (const c of candidates) {
            const h = await this.env.BEDROCK.head(c);
    ...
    this.server.tool(
      ...
      async ({ prefix, minCount }) => {
        ...
        await scanTextFiles(this.env.BEDROCK, prefix, (_k, text) => {
    ...
    this.server.tool(
      ...
      async ({ tag, prefix, limit }) => {
        ...
        const matches = await scanTextFiles(this.env.BEDROCK, prefix, (k, text, o) => {
    ...
    this.server.tool(
      ...
      async ({ template, target, vars, overwrite }) => {
        const tpl = await this.env.BEDROCK.get(template);
        if (!tpl) return err(`模板不存在：${template}`);
        if (!overwrite) {
          const existed = await this.env.BEDROCK.head(target);
        ...
        const ct = target.toLowerCase().endsWith(".md")
          ? "text/markdown; charset=utf-8"
          : "text/plain; charset=utf-8";
        await this.env.BEDROCK.put(target, rendered, { httpMetadata: { contentType: ct } });
    ...
    this.server.resource(
      ...
      new ResourceTemplate("bedrock://doc/{key}", {
        list: async () => {
          const r = await this.env.BEDROCK.list({ limit: 1000 });
          return {
      ...
      async (uri, { key }) => {
        const obj = await this.env.BEDROCK.get(key as string);
    ...
    this.server.resource(
      ...
      async (uri) => {
        const counter = new Map<string, number>();
        await scanTextFiles(this.env.BEDROCK, undefined, (_k, text) => {
          for (const t of extractTags(text)) counter.set(t, (counter.get(t) ?? 0) + 1);
          return null;
        });
        const items = [...counter.entries()]
    ...
    this.server.resource(
      ...
      new ResourceTemplate("bedrock://daily/{date}", {
        list: async () => {
          const r = await this.env.BEDROCK.list({ prefix: "日记/", limit: 1000 });
          return {
            resources: r.objects
...
// 204/205/304 按 spec 不能带 body，否则 new Response 会抛错
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === "/mcp") {
      const res = await BedrockMCP.serve("/mcp").fetch(req, env, ctx);
      return withCors(res);
    }
