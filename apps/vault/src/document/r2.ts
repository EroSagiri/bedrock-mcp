...
export async function scanTextFiles<T>(
  bucket: VaultDocuments,
  prefix: string | undefined,
  fn: (key: string, text: string, obj: Awaited<ReturnType<VaultDocuments["get"]>> extends infer TDoc ? Exclude<TDoc, null> : never) => T | null | Promise<T | null>,
  ...
  outer: do {
    ...
    const targets = r.items.filter(o => isTextDocumentKey(o.key));
